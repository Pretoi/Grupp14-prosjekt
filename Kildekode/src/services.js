...
class EventService {
  ...
  getUserInfo1(currentArrID, callback) {
    connection.query('SELECT Medlemsnr FROM Arrangement_Rolle WHERE Arrangements_ID = ? AND Medlemsnr IS NOT NULL', [currentArrID], (error, result) => {
      if (error) throw error
  ...
  getAvailableUsers(Arrangements_ID, currentArrDato, callback){
    ...
    [Arrangements_ID, currentArrDato], (error, result) => {
      if (error) throw error
